document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildItemsContainer = document.querySelector('.build-items'); // container for selected items

    // --- Function to render selected build items ---
    function renderBuildItems() {
        buildItemsContainer.innerHTML = ''; // clear previous items
    ...
    Object.keys(partsData).forEach(part => {
        ...
        if (selectElem) {
            ...
            selectElem.addEventListener('change', () => {
                ...
                checkCompatibility();
                updateTotalPrice();
                renderBuildItems(); // ✅ update the list whenever a selection changes
            });
        }
    });
